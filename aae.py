...
class AAE(train.AE):
    ...
    def model(self, latent, depth, scales, adversary_lr, disc_layer_sizes):
        ...
        adversary_loss_latents = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=adversary_logit_latent,
                                                    labels=tf.zeros_like(adversary_logit_latent)
                                                    )
                                                )
        # loss on real prior h
        adversary_loss_prior = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=adversary_logit_prior,
        ...
        ae_vars = tf.global_variables('aae_enc') + tf.global_variables('aae_dec')
